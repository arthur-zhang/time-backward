...
/// This crate has a runtime dependency on artifacts produced by the `time-backward-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "time-backward-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `time-backward-ebpf`, so cargo will automatically rebuild us
        // if `time-backward-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `time-backward-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("time-backward-ebpf");
